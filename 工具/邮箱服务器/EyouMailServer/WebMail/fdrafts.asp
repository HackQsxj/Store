...
<table width="100%" border="0" align="center" cellspacing="0" bordercolor=gray  bordercolordark=white  >
  <tr bgcolor="#dcdcdc"  valign="left"> 
	...
	If Session("LoginSuccess")= 1 Then
		...
		strSql = "select * from mailfolder where account='" & Session("Account") & "' and folderid='5' order by mailid DESC"
		rs.open strSql,Conn,1,2
%>
	<form action="" method=post align="left">
	<table width="100%">
          <tr>
            <td valign=center width="100%" height="1"><%=IDS_DRAFTS%>&nbsp;<%=RS.recordcount%> 
		...
		nCurCur = 1
  		...
  		if Request.QueryString("pages")<>"" then
		    	if not IsNumeric(Request.QueryString("pages")) then response.Redirect "fdrafts.asp"
			...
			For nI = 1 To nCurNo - 1
		  		RS.movenext
		  		if RS.eof then response.Redirect "fdrafts.asp"
			Next
...
%>
	...
	%>
            ...
            <td><a href='fgetmail.asp?subpage=fdrafts&indexOfMail=<%=HTMLEncode(RS("uid"))%>' > 
              <%if rs("mailIsread") = 0 then %>
              <b> 
...
%>
        ...
        <input type= hidden value="fdrafts" name="subpage" id="subpage">
  ...
  <tr bgcolor="#dcdcdc"  valign="left"> 
    <td valign="top" bgcolor="#FFFFF7"  height="100%"> 
      <div align="center">>> 
        ...
        &nbsp; 
        <%if nI<> nCurPage then %>
        <a href="fdrafts.asp?pages=<%=nI%>"><%=nI%></a> 
        <% else %>
        <b><%=nI%></b> 
        <%end if%>
        &nbsp; 
