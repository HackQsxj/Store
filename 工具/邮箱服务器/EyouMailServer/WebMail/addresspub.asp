...
<table width="100%" border="0" align="center" cellspacing="0" bordercolor=gray  bordercolordark=white>
	...
	<%
		...
		If Session("LoginSuccess") = 1 Then
			...
			If InStr(Session("Account"), "@") > 0 Then
				strSql = "select * from address where account='Admin@" & Session("Domain") & "' order by name"
			Else
				strSql = "select * from address where account='Admin' order by name"
			End If
	...
	%>&nbsp; 
	<%If nI<> nCurPage Then%>
	<a href="addresspub.asp?pages=<%=nI%>"><%=nI%></a>
	<%Else%>
				<b><%=nI%></b> 	
